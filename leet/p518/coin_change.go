...
func change(amount int, coins []int) int {
	...
	for i := 1; i <= amount; i++ {
		for j := 0; j < len(coins); j++ {
			...
			if i-cj > 0 {
				//r := 0
				for k := 0; k < len(coins) && coins[k] <= cj; k++ {
					//ck := coins[k]
					if m[i-cj][k] > 0 {
						m[i][j] += m[i-cj][k]
					}
